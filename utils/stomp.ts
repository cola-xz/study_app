...
function onSocketOpen(): void {
	...
	if (c.heartbeat && c.heartbeat > 0)
		hdrs.push(`heart-beat:${Math.round(c.heartbeat / 2)},${Math.round(c.heartbeat / 2)}`);
...
export function send(
	destination: string,
	body?: string | Record<string, unknown> | number | boolean,
	headers?: Record<string, string>
): void {
	...
	const hdrs: string[] = [
		...
		`content-length:${textLength(text)}`,
	];
...
export function subscribe(
	...
	for (const k of Object.keys(headers || {}))
		hdrs.push(`${k}:${encodeValue((headers as any)[k])}`);
	pushText('SUBSCRIBE\n' + hdrs.join('\n') + '\n\n' + FRAME_SEP);
	return id;
}
