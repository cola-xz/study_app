				...
				<view class="tree-panel">
					<view class="panel-head">
						...
						<text class="panel-sub">
							<text v-if="isDemoData" class="panel-sub-demo">演示数据 · </text
							>{{ dirCount }} 个目录 · {{ videoCount }} 个视频
						</text>
					</view>
				...
				<view class="popup-head">
					<view class="popup-title-row">
						...
						<text class="popup-title">{{
							popupSelected.name || displayName(popupSelected.raw)
						}}</text>
					</view>
					<text class="popup-sub">{{
						popupSelected.displayPath || displayRawPath(popupSelected.raw)
					}}</text>
				...
				<view class="popup-body">
					...
					<view class="popup-detail">
						<view class="popup-detail-row">
							...
							<text class="popup-detail-value">{{
								displayName(popupSelected.raw)
							}}</text>
						</view>
						<view class="popup-detail-row">
							<text class="popup-detail-label">文件格式</text>
							<text class="popup-detail-value">{{
								formatType(popupSelected.raw)
							}}</text>
						</view>
...
const DEMO_DIRS: UploadChild[] = [
	...
	{
		id: 'demo-concert-dengziqi',
		parentId: 'demo-concert',
		childPath: 'dengziqi',
		pathName: '邓紫棋',
	},
...
const DEMO_VIDEOS: VideoItem[] = [
	{
		...
		fileSuffix: '.mp4',
		fileByName: '邓紫棋演唱会现场',
		childrenPath: '/concert/dengziqi',
		fileDesc: '邓紫棋演唱会精彩现场实录。',
	},
	{
		fileName: '测试片段',
		fileSuffix: '.mp4',
		fileByName: '测试片段',
		childrenPath: '/test/test2',
		fileDesc: '演示用测试视频片段。',
	},
];
...
function buildTree(dirList: UploadChild[], files: VideoItem[]): TreeNodeData[] {
	...
	const vidId = (item: VideoItem, suffix: string) =>
		`video__${suffix}__${item.key || item.fileName}${
			item.fileNameSuffix || item.fileSuffix || ''
		}`;
	...
	for (const item of files) {
		...
		for (const seg of segs) {
			const childDir = parent.children.find(
				(c) => c.isDir && (c.path === seg || c.name === seg)
			);
			if (childDir) {
