<template>
	<view class="app-layout">
		<!-- 顶部导航栏 -->
		<view class="nav-bar">
			<uni-nav-bar height="85rpx" color="#999" backgroundColor="#ffffff" shadow>
				<view class="nav-bar-title" :class="{ 'nav-bar-title-dropdown': childMenus.length }"
					@click="toggleDropdown">
					<text class="nav-bar-title-text">{{ title }}</text>
					<text v-if="childMenus.length" class="dropdown-title-arrow"
						:class="{ 'dropdown-trigger-arrow-open': dropdownOpen }">▾</text>
				</view>
				<view v-if="childMenus.length && dropdownOpen" class="dropdown-panel" @click="dropdownOpen = false">
					<view v-for="menu in childMenus" :key="menu.path" class="dropdown-item"
						:class="{ 'dropdown-item-active': isActive(menu) }" @click.stop="handleMenuTap(menu)">
		...
		<view v-if="tabList.length" class="bottom-menu">
			<view v-for="item in tabList" :key="item.path" class="bottom-menu-item"
				:class="{ 'bottom-menu-item-active': isActive(item) }" @click="handleMenuTap(item)">
